					...
					if(stmt instanceof CreateTable){
						...
						if (select instanceof PlainSelect){
							...
							for(int temp = 0; temp<=unionItems.size()-1;temp++){
								...
								if(unionItems.get(temp).getHaving() != null)
									oper = new GroupOperator(oper, fromScan.schemaCol,unionItems.get(temp).getSelectItems(), unionItems.get(temp).getGroupByColumnReferences(),unionItems.get(temp).getHaving());
								else if(unionItems.get(temp).getGroupByColumnReferences() != null)
									oper = new GroupOperator(oper, fromScan.schemaCol,unionItems.get(temp).getSelectItems(), unionItems.get(temp).getGroupByColumnReferences());
								else
